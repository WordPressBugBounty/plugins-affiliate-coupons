...
function affcoups_add_shortcode( $atts, $content ) {
    ...
    //Set options to null if not set already to avoid notice in frontend when debug mode is on
    ...
    // 2. 'hide_expired_coupons' setting is enhanced via radio buttons now: 'Show all coupons' / 'Show active coupons only'
    } elseif ( isset( $options['hide_expired_coupons'] ) && '1' === $options['hide_expired_coupons'] ) {
        $args['affcoups_coupon_expired'] = 0;
    }
      $coupons = affcoups_get_coupons( $args );
      $modal_popup_coupons = affcoups_pro_modal_popup ($coupons);
	...
	ob_start();
    //prepare for popup modal when pagination or social share is active
    if($options['pagination'] || $options['social_share'])
      echo $modal_popup_coupons;
    if ( $coupons ) {
        //-- Apply filters
        ...
        // since pro ver 1.1.13 show expiry countdown
        if ( isset( $atts['show_expiry_countdown'] ) && in_array( $atts['hide_show_expiry_countdown'], array( 'true', 'false' ) ) ) {
            $affcoups_template_args['show_expiry_countdown'] = $atts['show_expiry_countdown'];
        }

        // since pro ver 1.1.12 hide image option
        if ( isset( $atts['hide_image'] ) && in_array( $atts['hide_image'], array( 'true', 'false' ) ) ) {
            $affcoups_template_args['hide_image'] = $atts['hide_image'];
        }
        ...
        if ( isset( $atts['code'] ) ) {
         ...
         //since ver 1.6.7.8 to hide description
        if ( isset( $atts['description'] ) ) {
            $affcoups_template_args['description'] = $atts['description'];
        }else
            $affcoups_template_args['description'] = '';

         //since ver 1.6.7.8 to hide title
         if ( isset( $atts['title_display'] ) ) {
            $affcoups_template_args['title_display'] = $atts['title_display'];
        } else
        $affcoups_template_args['title_display'] = '';
